...
import GpuData from '../../../web-scraping-api/controllers/updateAllHardwareData.json'; // Adjust path as necessary

export default function GpuCategoryScreen({ route, navigation }) {
  // Destructure category with a default empty string
  const { category = '' } = route.params || {};

  // Validate and filter data based on category
  const gpuData = GpuData ? GpuData.filter(item =>
    item.productName.includes(category) && item.productName.includes('Placa de Vídeo')
  ) : [];
  ...
  const handleOptionSelect = (item) => {
    navigation.navigate('DetailScreen', { item, type: 'GPU' });
  };

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Select a GPU {category} Option</Text>
      <FlatList
        data={gpuData}
